...
// Arquivo: scripts/busca-script.js
...
function filtrarMateriais() {
    ...
    filteredMateriais = materiaisData.filter(material => {
        // Pega Título e Autor, garantindo que não quebre se for nulo
        const title = (material.Titulo || '').toLowerCase();
        const author = (material.Autor || '').toLowerCase(); // <--- NOVO!
        const local = material.Localizacao || '';

        // Verifica se o termo digitado está no Título OU no Autor
        const matchText = title.includes(searchTerm) || author.includes(searchTerm); // <--- ATUALIZADO
